...
# Evaluation utils
##########################
def np_pose_error(est_pose, gt_pose):
    """
    Compute the error between and estimated and groud-truth poses
    """
    t_err = np.linalg.norm(est_pose[:3]-gt_pose[:3])
    quat_R_err = 2*np.arccos(abs(np.dot(est_pose[3:],gt_pose[3:])))*180/np.pi
    return t_err, quat_R_err
...
netvlad_transforms = transforms.Compose([transforms.ToTensor(),
